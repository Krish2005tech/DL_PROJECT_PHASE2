...
# Initialize Standard Vim with 4 classes (0, 90, 180, 270 degrees)
model = VimModel(num_classes=4).to(device)
...
print("🚀 Starting Rotation Training for Baseline: Vim...")
...
for epoch in range(10):
    ...
    print(f"[Vim Baseline] Epoch {epoch}: Loss={loss:.4f}, Acc={acc:.4f}")

total_time = time.time() - start_time

# Save results for the final comparison graph
with open("results/vim_rotation_results.json", "w") as f:
    json.dump({"history": history, "total_time": total_time}, f)

print("✅ Vim Baseline Rotation Training Completed")
